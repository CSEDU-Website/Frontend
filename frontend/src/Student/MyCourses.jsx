...
const MyCourses = () => {
  ...
  useEffect(() => {
    const fetchMyCourses = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.get(`${BACKEND_URL}/v1/student/courses/my_classes/${studentProfile.id}`);
  ...
  const fetchAssignments = async (courseId) => {
    setLoading(true);
    try {
      const response = await axios.get(`${BACKEND_URL}/v1/student/courses/assignments/get/all?course_id=${courseId}`);
      ...
      const submissionsMap = submissionResults.reduce((acc, curr) => ({ ...acc, ...curr }), {});
  ...
  const uploadFiles = async (files) => {
    const uploadPromises = files.map(async (file) => {
      ...
      try {
        ...
        const response = await axios.post(`${BACKEND_URL}/v1/utility/upload`, formData, {
          headers: {
  ...
  const handleSubmissionUploadWithFiles = async (assignmentId) => {
    ...
    const manualLinks = fileLinks.trim().split(',').map(link => link.trim()).filter(link => link);
    
    if (files.length === 0 && manualLinks.length === 0) {
      setMessage({ type: 'error', text: 'Please select files or provide file links' });
      return;
    }
    ...
    try {
      ...
      if (submissions[assignmentId]) {
        ...
        response = await axios.post(`${BACKEND_URL}/v1/student/courses/update/submission`, submissionData);
      } else {
        // Create new submission
        response = await axios.post(`${BACKEND_URL}/v1/student/courses/create/submission`, submissionData);
      }
      ...
      const errorMessage = error.response?.data?.detail || error.message || 'Failed to upload submission';
  ...
  const handleLeaveCourse = async (courseId) => {
    ...
    setLoading(true);
    try {
      await axios.get(`${BACKEND_URL}/v1/student/courses/leave_class/${studentProfile.id}/${courseId}`);
      ...
      const errorMessage = error.response?.data?.detail || 'Failed to leave course';
      setMessage({ type: 'error', text: errorMessage });
    } finally {
      setLoading(false);
    }
        ...
        {selectedCourse && activeTab === 'assignments' && (
          <div className="rounded-2xl shadow-lg overflow-hidden mb-8 transform transition-all duration-300 hover:shadow-xl">
            <div 
              ...
              style={{ 
                backgroundImage: `url(${selectedCourse.image_url || 'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1200&q=80'})` 
              }}
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                ...
                <h1 className="text-3xl sm:text-4xl font-bold mb-3 text-shadow">{selectedCourse.title}</h1>
          ...
          <div>
            ...
            <div className="mb-6 bg-white rounded-xl shadow-md p-6">
              <div className="relative">
                <svg xmlns="http://www.w3.org/2000/svg" className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {filteredCourses.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCourses.map((course) => (
                  <div key={course.id} className="bg-white border border-gray-200 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
                    {/* Course Image */}
                    <div 
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                        ...
                        <h3 className="text-xl font-bold mb-1 text-shadow truncate">{course.title}</h3>
                      </div>
                    </div>
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4 text-xs">
                        ...
                        <span className={`px-2.5 py-1 rounded-full border shadow-sm ${
                          course.type === 'Theory' ? 'bg-blue-100 text-blue-800 border-blue-200' : 'bg-green-100 text-green-800 border-green-200'
                        }`}>
                          {course.type}
                        </span>
                      </div>
                      ...
                      {course.schedules && course.schedules.length > 0 && (
                        <div className="mb-4">
                          <p className="text-xs text-gray-500 mb-2 font-medium">Schedule:</p>
                          <div className="flex flex-wrap gap-1.5">
                            {course.schedules.map((schedule, index) => (
                              <span key={index} className="text-xs bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-full border border-indigo-200 shadow-sm">
              ...
              <div className="p-6">
                ...
                {assignments.length === 0 ? (
                  <div className="text-center py-16 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    ...
                    <h3 className="text-xl font-medium text-gray-700 mb-2">No assignments yet</h3>
                    <p className="text-gray-500 max-w-md mx-auto">Assignments will appear here when posted by your instructor</p>
                  </div>
                ) : (
                  <div className="space-y-8">
                    {assignments.map((assignment) => {
                      ...
                      return (
                        <div key={assignment.id} className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-4">
                              <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                                  ...
                                  {assignment.due_date && (
                                    <div className={`flex items-center rounded-full px-3 py-1.5 shadow-sm ${
                                      overdue && !submission 
                                      ...
                                      {overdue && !submission && <span className="ml-1">(Overdue)</span>}
                                    </div>
                                  )}
                            ...
                            {assignment.file_links && Object.keys(assignment.file_links).length > 0 && (
                              <div className="mb-5 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {Object.entries(assignment.file_links).map(([fileName, url]) => (
                                    <a
                                      key={fileName}
                                      href={url}
                                      target="_blank"
                            ...
                            <div className="border-t border-gray-200 pt-5 mt-4">
                              {submission ? (
                                <div className="bg-green-50 border border-green-200 p-5 rounded-xl shadow-inner">
                                  ...
                                  {submission.file_links && Object.keys(submission.file_links).length > 0 && (
                                  ...
                                  {submission.checked && submission.marks !== null && (
                                  ...
                                  <div className="mt-4 ml-9">
                                    <p className="text-sm text-green-700 mb-3 font-medium">Update Submission:</p>
                                    
                                    {/* File Upload Section */}
                                    <div className="space-y-3 mb-4">
                                      <div>
                                        <label className="block text-xs text-green-600 mb-1 font-medium">Upload Files:</label>
                                        <input
                                          type="file"
                                          multiple
                                        ...
                                        {selectedFiles[assignment.id] && selectedFiles[assignment.id].length > 0 && (
                                          <div className="mt-2">
                                            <div className="flex items-center justify-between mb-1">
                                              <p className="text-xs text-green-600 font-medium">Selected files:</p>
                                              <button
                                                onClick={() => clearSelectedFiles(assignment.id)}
                                            ...
                                            <div className="flex flex-wrap gap-1">
                                              {selectedFiles[assignment.id].map((file, index) => (
                                                <span key={index} className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded border border-green-200 flex items-center gap-1">
                                      ...
                                      <div className="text-xs text-green-600 text-center">OR</div>
                                      
                                      <div>
                                        <label className="block text-xs text-green-600 mb-1 font-medium">Manual File Links:</label>
                                        <input
                                          type="text"
                                          placeholder="Enter file links (comma-separated)"
                                          value={uploadingAssignment === assignment.id ? fileLinks : ''}
                                          onChange={(e) => {
                                            if (uploadingAssignment === assignment.id) {
                                              setFileLinks(e.target.value);
                                            }
                                          }}
                                          onFocus={() => setUploadingAssignment(assignment.id)}
                                    ...
                                    <button
                                      onClick={() => handleSubmissionUploadWithFiles(assignment.id)}
                                      disabled={uploadingAssignment === assignment.id && !fileLinks.trim() && (!selectedFiles[assignment.id] || selectedFiles[assignment.id].length === 0)}
                                ...
                                <div className={`p-5 rounded-xl border-2 border-dashed ${
                                  overdue ? 'border-red-300 bg-red-50' : 'border-indigo-300 bg-indigo-50'
                                } shadow-inner`}>
                                  <div className="text-center mb-5">
                                    <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-3 ${
                                      overdue ? 'bg-red-100 text-red-500' : 'bg-indigo-100 text-indigo-500'
                                    }`}>
                                      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                      </svg>
                                    </div>
                                    <p className={`text-lg font-medium ${overdue ? 'text-red-800' : 'text-indigo-700'}`}>
                                      {overdue ? 'Assignment Overdue' : 'Submit Your Assignment'}
                                    </p>
                                    <p className={`text-sm mt-1 ${overdue ? 'text-red-600' : 'text-indigo-600'}`}>
                                      {overdue ? 'You can still submit, but it will be marked as late' : 'Upload your work before the deadline'}
                                    </p>
                                  </div>
                                  ...
                                  <div className="space-y-4">
                                    {/* File Upload Section */}
                                    <div>
                                      <label className={`block text-sm font-medium mb-2 ${overdue ? 'text-red-700' : 'text-indigo-700'}`}>
                                        Upload Files:
                                      </label>
                                      <input
                                        type="file"
                                      ...
                                      {selectedFiles[assignment.id] && selectedFiles[assignment.id].length > 0 && (
                                        <div className="mt-2">
                                          <div className="flex items-center justify-between mb-1">
                                            <p className={`text-xs mb-1 ${overdue ? 'text-red-600' : 'text-indigo-600'}`}>
                                              Selected files:
                                            </p>
                                            <button
                                              onClick={() => clearSelectedFiles(assignment.id)}
                                              className={`text-xs hover:underline ${overdue ? 'text-red-600 hover:text-red-800' : 'text-indigo-600 hover:text-indigo-800'}`}
                                            >
                                              Clear all
                                            </button>
                                          </div>
                                          <div className="flex flex-wrap gap-1">
                                            {selectedFiles[assignment.id].map((file, index) => (
                                              <span key={index} className={`text-xs px-2 py-1 rounded border flex items-center gap-1 ${
                                                overdue 
                                                  ? 'bg-red-100 text-red-700 border-red-200' 
                                                ...
                                                <span>{file.name}</span>
                                                <span className={overdue ? 'text-red-600' : 'text-indigo-600'}>({formatFileSize(file.size)})</span>
                                              </span>
                                            ))}
                                          </div>
                                        </div>
                                    ...
                                    <div className={`text-center text-sm font-medium ${overdue ? 'text-red-600' : 'text-indigo-600'}`}>
                                      OR
                                    </div>
                                    ...
                                    <button
                                      ...
                                      className={`w-full px-4 py-2.5 rounded-lg text-sm font-medium text-white shadow-sm hover:shadow transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${
                                        overdue ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'
                                      }`}
                                    >
                                      {uploadingFiles ? (
                                        <>
